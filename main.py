...
# Collect all photo and video files from respective directories
photos = glob("photos/*")
videos = glob("videos/*")
result = "result"
landmark_prediction_model="shape_predictor_68_face_landmarks.dat"

# Temporary folder for intermediate files; ensure no conflicts with existing directories
trash = "trash"
...
def main(img, img2, landmark_pts, triangle_index):
    # Detect and process the face in the frame (target image)
    faces2 = detector(img2)

    landmark_pts2 = []

    # Extract landmark points for the target image
    for face in faces2:
    ...
    # Define face region in the target image
    ...
    # Process triangles from the source and map them to the target image
...
if __name__ == "__main__":
    for photo in photos: # iterating through each photo
        ...
        # Detect face and extract landmarks in the source photo
